 ...
 *
 * ASSUMPTION: day_of_week is stored as integer 0=Monday … 6=Sunday
 *             meal_type is stored as text: 'lunch' or 'dinner'
 */
...
const DEMO_EMAIL = "demo-restaurant@tiffino.dev";
...
// Indexed by dish name; day 0=Mon, 6=Sun
...
async function run() {
  ...
  const { error: profileErr } = await supabase.from("profiles").upsert(
    { id: userId, role: "restaurant", email: DEMO_EMAIL, full_name: "Demo Owner",
      restaurant_name: "Shree Krishna Mess", area: "Kothrud, Pune" },
    { onConflict: "id" }
  );
  if (profileErr) { console.error("❌  profile upsert:", profileErr.message); process.exit(1); }
  ...
  if (existingRestaurant) {
    ...
    await supabase.from("restaurants").update({
      name: "Shree Krishna Mess", area: "Kothrud, Pune",
      address: "42, Paud Road, Kothrud, Pune 411038",
      base_price: 70, serves_lunch: true, serves_dinner: true,
      lunch_skip_cutoff: "10:00", dinner_skip_cutoff: "18:00",
    }).eq("id", restaurantId);
    ...
    if (error || !data) { console.error("❌  restaurant insert:", error?.message); process.exit(1); }
  ...
  const { data: insertedDishes, error: dishErr } = await supabase
    ...
    .select("id, name");
  if (dishErr || !insertedDishes) { console.error("❌  dishes insert:", dishErr?.message); process.exit(1); }
  ...
  if (menuErr) { console.error("❌  weekly_menu insert:", menuErr.message); process.exit(1); }
  console.log(`   ↳ ${menuRows.length} menu slots inserted (Mon–Sun, lunch + dinner)`);

  console.log("\n✅  Seed complete!");
  console.log(`   Restaurant ID : ${restaurantId}`);
  console.log(`   Public URL    : /restaurants/${restaurantId}`);
  console.log(`   Login as demo : /api/dev/test-auth?role=restaurant  (dev only)`);
}
